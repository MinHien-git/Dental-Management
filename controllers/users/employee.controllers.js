...
const add_patient_note = async (req, res) => {
    try {
        await sql.connect(config);
        const request = new sql.Request();
        request.input("@MaKHDT", "QUYASDB");
        request.input("@MaDT", "XKLHASDH");
        request.input("@MaBeMat", "JASNXYB");
        request.input("@MaRang", "XSDC");

        const result = request.execute("PROC_ThemChiTietKeHoachDieuTri", (err, result) => {
            console.log(result.output);
        });
        res.send("DB Connected");
    } catch (err) {
...
const edit_patient_note = async (req, res) => {
    try {
        await sql.connect(config);
        const request = new sql.Request();
        request.input("@mbn", "CCCDA");
...
const get_patient_payment = async (req, res) => {
    try {
        await sql.connect(config);
        const request = new sql.Request();
        const result = request.execute("PROC_XemDanhSachThanhToanCuaBenhNhan", (err, result) => {
            console.log(result.output);
        });
        res.send("DB Connected");
    } catch (err) {
